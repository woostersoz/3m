...
from websites.serializers import TrafficSerializer
...
def filterWebsites(request, id):
    ...
    try:   
        ...
        if code is  None:
            ...
            result = filterWebsitesGoog(user_id=user_id, company_id=company_id, start_date=start_date, end_date=end_date, visitor_type=visitor_type, series_type=series_type, query_type=query_type, page_number=page_number, items_per_page=items_per_page, system_type=system_type, offset=offset, code=code, chart_name=chart_name, filters=filters)
        elif code == 'sfdc': 
            pass
...
def filterWebsitesGoog(user_id, company_id, start_date, end_date, visitor_type, series_type, query_type, page_number, items_per_page, system_type, offset, code, chart_name, filters):    
...
#                 #querydict = {company_field_qry: company_id} #, lead_type_qry: True
        if query_type == "strict": #we are not done
            ...
            results_qry = '$results.' + visitor_type + '.' + series_type
            ...
            if analyticsIds is None:
                return []
            print 'visitor_type is ' + visitor_type[-3:]
            visitor_type = visitor_type[-3:]
            if visitor_type != 'New':
                visitor_type = 'Returning'
            ids = analyticsIds['results'][filterProfile].get(visitor_type, None)
            print 'ids is ' + str(ids)
            visitors = Traffic.objects(id__in=ids).skip(offset).limit(items_per_page).order_by('data.ga:hour', 'data.ga:minute')
            #print 'start5 is ' + str(time.time())
            #now do the calculations
            total = Traffic.objects(id__in=ids).count() #len(leads)
            #print 'start6 is ' + str(time.time())
        
        serializer = TrafficSerializer(visitors, many=True)   
        return JsonResponse({'count' : total, 'results': serializer.data})   
    except Exception as e:
        return JsonResponse({'Error' : str(e)})
