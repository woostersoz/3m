...
def hspt_campaign_email_performance(user_id, company_id, start_date, end_date, chart_name, filters): 
    try:
        ...
        #print 'filters are ' + str(json.loads(filters))   
        filterCampaign = json.loads(filters).get('campaign_guid', None)
        #filterCampaign = filterCampaign.get('id', None)
        #print 'fc is ' + str(filterCampaign)
        ...
        while s < (e - delta):
            ...
            campaign_values = data[campaign_name]
                        ...
                        for result in results:
                            if result['key'] != key:
                                ...
                                if email_found == False:
                                    result['values'].append({'label': email['name'] , 'value': value, 'email_id': email['id']}) #+ '-' + str(email['id'])
...
def hspt_email_cta_performance(user_id, company_id, start_date, end_date, chart_name, filters): 
    try:
        results = []
        ...
        delta = timedelta(days=1)
        values = []
        result = {'key': 'CLICK', 'values': values}
        results = [result]
        ...
        while s < (e - delta):
            ...
            if existingData is None: #  no results found for day so move to next day
                continue
            ctas = existingData['results'] #contains an array of CTA click counts for that day

            for cta in ctas:
                url = cta['url'] 
                value_found = False
                for value in values: #check if this url already exists as an entry in the values list
                    if value['url'] == url: #this url exists so increase the count
                        value['value'] += cta['count']
                        value_found = True
                        break
                if value_found == False:
                    new_value = {}
                    new_value['url'] = url
                    new_value['label'] = cta['label']
                    new_value['value'] = cta['count']
                    values.append(new_value)
        
        new_values = sorted(values, key=itemgetter('value'), reverse=True)  
        new_values = new_values[0:20]
